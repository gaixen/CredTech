...
This project implements a comprehensive credit risk modeling system for Credit Default Swap (CDS) spread prediction, following academic methodologies from:
...
Implements feature engineering following Das et al. (2009) methodology:
...
Comprehensive modeling framework with:
...
2. **Rolling Averages**: 4-quarter rolling means for key ratios
3. **Winsorization**: 1% level to control extreme values
4. **Standardization**: Z-score normalization for model input
5. **Interaction Terms**: ROA×Leverage, Growth×Volatility
